import React from "react";
import Card from "../Card/Card";
import "./cardsList.scss";

export interface IProductDetails {
  details: IProduct;
}

interface IProduct {
  id: number;
  price: string;
  scores: number;
  img: string;
  title: string;
}

export default class CardsList extends React.Component {
  state: IProduct[] = [
    {
      id: 0,
      price: "15.90",
      scores: 2459,
      img: "https://m.media-amazon.com/images/I/61Zh467pKjL._AC_SX522_.jpg",
      title:
        "Apple AirPods (2nd Generation) Wireless Earbuds with Lightning Charging Case Included. Over 24 Hours of Battery Life, Effortless Setup. Bluetooth Headphones for iPhone",
    },
    {
      id: 1,
      price: "373.90",
      scores: 1540,
      img: "https://m.media-amazon.com/images/I/61QGMX0Qy6L._AC_SX679_.jpg",
      title:
        "Lenovo 2022 Newest Ideapad 3 Laptop, 15.6 HD Touchscreen, 11th Gen Intel Core i3-1115G4 Processor, 8GB DDR4 RAM, 256GB",
    },
    {
      id: 2,
      price: "223.70",
      scores: 1043,
      img: "https://m.media-amazon.com/images/I/817aOXLoNpL._AC_SX679_.jpg",
      title:
        "SAMSUNG Galaxy S22 Ultra Cell Phone, Factory Unlocked Android Smartphone, 128GB, 8K Camera & Video, Brightest Display Screen, S Pen, Long Battery Life, Fast 4nm Processor, US Version, Phantom White",
    },
    {
      id: 3,
      price: "433.70",
      scores: 1021,
      img: "https://m.media-amazon.com/images/I/51Q0I+lvW9L._AC_SX466_.jpg",
      title:
        "DJI Osmo Mobile 6 Smartphone Gimbal Stabilizer, 3-Axis Phone Gimbal, Built-In Extension Rod, Portable and Foldable, Android and iPhone Gimbal with ShotGuides, Vlogging Stabilizer, YouTube TikTok Video",
    },
    {
      id: 3,
      price: "433.70",
      scores: 1021,
      img: "https://m.media-amazon.com/images/I/711SkUWCQVL._AC_SX679_.jpg",
      title:
        "Techni Mobili Sit-to-Stand Mobile Medical Laptop Computer Cart, Black, Adjustable Height",
    },
  ];

  render() {
    return (
      <>
        <div>Items found: {this.state.length}</div>
        <ul className={"cards-list"}>
          {this.state.map((card, index) => (
            <Card key={index} details={card} />
          ))}
        </ul>
      </>
    );
  }
}
